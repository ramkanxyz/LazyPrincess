...
BOT_TOKEN = environ['6733525602:AAHSlqN955nMrcu2Xj31QHJOOL49nTIJloU']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('5767113882', '').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002152575794').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('5767113882', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('/rfggjj')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search( ) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URI = environ.get('mongodb+srv://krahul724855:<crazy&xyz>@cluster0.mj7zqwi.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0', "")
DATABASE_NAME = environ.get('DATABASE_NAME', "krahul724855")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'Telegram_files')

# Others
LOG_CHANNEL = int(environ.get('-1002152575794', 0))
...
URL_SHORTENR_WEBSITE = environ.get('gplinks.com', '') #Always use website url from api section 
